...
export default function TelaHoje(){
    ...
    useEffect(() => {
        ...
        .then(res => {
            ...
            setQtdHabitosFeitos(res.data.filter(elem => elem.done === true).length);
            setHabitosCompletados((res.data.length === 0 || qtdHabitosFeitos === 0) ? 0 : Math.round((qtdHabitosFeitos/listaHabitosHoje.length)*100));
        })
        .catch(err => {
            console.log(err);
            alert("Ocorreu um erro durante o carregamento dos hábitos de hoje. Por favor, tente novamente...");
        });
    }, [listaHabitosHoje]);
    ...
    function toggleCheck(id, done){
        ...
        if (done){
            ...
            .catch(err => {
                console.log(err);
                alert("Ocorreu um erro durante a operação de desmarcar o hábito. Por favor, tente novamente...");
            });
        }else{
            axios
            ...
            .catch(err => {
                console.log(err);
                alert("Ocorreu um erro durante a operação de marcar o hábito. Por favor, tente novamente...");
            });
        }
    }
        ...
        <>
            ...
            <ContainerPrincipal>
                <p  data-test="today">{diaAtual[0].toUpperCase() + diaAtual.slice(1)}</p>
                {
                    ...
                    <p data-test="today-counter" style={{color: '#8FC549'}}>{Math.round((qtdHabitosFeitos/listaHabitosHoje.length)*100)}% dos hábitos concluídos</p>
                }
                        ...
                        <div data-test="today-habit-container" key={elem.id}>
                            <div>
                                ...
                                <CurrentSequenceText 
                                    ...
                                    cor={(elem.done) ? "#8FC549" : "#666666"}>
                                    <span style={{color: '#666666'}}>Sequência atual:</span> {elem.currentSequence} {(elem.currentSequence === 1) ? "dia" : "dias"} 
                                </CurrentSequenceText>
                                <HighestSequenceText 
                                    data-test="today-habit-record" 
                                    cor={(elem.currentSequence === elem.highestSequence && elem.done) ? "#8FC549" : "#666666"}>
                                    <span style={{color: '#666666'}}>Seu recorde:</span> {elem.highestSequence} {(elem.highestSequence === 1) ? "dia" : "dias"}
                                </HighestSequenceText>
                            </div>
